...
<div class="user-face">
<a href="<?php echo SITE_URL;?><?php echo ikurl('hi','',array('id'=>$strTopic['user'][doname]))?>"><img title="<?php echo $strTopic['user'][username];?>" alt="<?php echo $strTopic['user'][username];?>" src="<?php echo $strTopic['user'][face];?>" width="48"></a>
<br />
...
<span class="pl20">来自：<a href="<?php echo SITE_URL;?><?php echo ikurl('hi','',array('id'=>$strTopic['user'][doname]))?>"><?php echo $strTopic['user'][username];?></a></span>
</h3>
...
&gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=group&ac=do&ik=topic_istop&topicid=<?php echo $strTopic['topicid'];?>"><?php if($strTopic['istop']=='0') { ?>置顶<?php } else { ?>取消置顶<?php } ?></a> 

&gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=group&ac=do&ik=isposts&topicid=<?php echo $strTopic['topicid'];?>"><?php if($strTopic['isposts']==0) { ?>精华<?php } else { ?>取消精华<?php } ?></a>

&gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=group&ac=do&ik=topic_isshow&topicid=<?php echo $strTopic['topicid'];?>"><?php if($strTopic['isshow']=='0') { ?>隐藏<?php } else { ?>显示<?php } ?></a>
&gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=group&ac=topic_move&groupid=<?php echo $strGroup['groupid'];?>&topicid=<?php echo $strTopic['topicid'];?>">移动</a>

<?php } ?>
&gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=group&ac=topic_edit&topicid=<?php echo $strTopic['topicid'];?>">编辑</a> &gt;&nbsp; 
<a  href="<?php echo SITE_URL;?>index.php?app=group&ac=do&ik=deltopic&topicid=<?php echo $strTopic['topicid'];?>" onclick="return confirm('确定删除?')">删除</a>
</div>
<?php } ?>
</div>
</div>
...
<?php if($IK_USER['user']) { ?>
<div><a class="share" href="javascript:void('0');" onclick="topic_collect('<?php echo $strTopic['topicid'];?>');">收藏</a></div>
<?php } ?>
<div class="clear"></div>
...
<input class="tagtxt" type="text" name="tags" id="tags" /> <button type="submit" class="subab" onclick="savaTag(<?php echo $topicid;?>)">添加</button> <a href="javascript:void(0);" onclick="showTagFrom()">取消</a>
</p>
<?php } ?>
...
<div class="clear"></div>
<div>
<?php if($upTopic) { ?>上一篇：<a href="<?php echo SITE_URL;?><?php echo ikurl('group','topic',array('id'=>$upTopic['topicid']))?>"><?php echo $upTopic['title'];?></a><?php } ?>

<?php if($downTopic) { ?>下一篇：<a href="<?php echo SITE_URL;?><?php echo ikurl('group','topic',array('id'=>$downTopic['topicid']))?>"><?php echo $downTopic['title'];?></a><?php } ?>
</div>
...
<p style=" padding:5px 0px"><button onclick="recomment(<?php echo $item['commentid'];?>,<?php echo $item['topicid'];?>)" id="recomm_btn_<?php echo $item['commentid'];?>" class="subab">提交</button>&nbsp;&nbsp;<a href="javascript:;" onclick="$('#rcomment_<?php echo $item['commentid'];?>').slideToggle('fast');">取消</a>
</p>
...
<div class="cright">
    ...
    <?php if($isGroupUser > 0 ) { ?>
    <p class="pl2">&gt; <a href="<?php echo SITE_URL;?><?php echo ikurl('group','add',array('groupid'=>$strGroup['groupid']))?>">在该小组发布帖子</a></p>
    <?php } else { ?>
    <p class="pl2">&gt; <a href="<?php echo SITE_URL;?><?php echo ikurl('group','do',array('ik'=>'join','groupid'=>$strGroup['groupid']))?>">加入这个小组</a></p>
    <?php } ?>
...
<li><a title="<?php echo $item['title'];?>" href="<?php echo SITE_URL;?><?php echo ikurl('group','topic',array('id'=>$item['topicid']))?>"><?php echo $item['title'];?></a> &nbsp;
<span class="pl">(<a href="<?php echo SITE_URL;?><?php echo ikurl('hi','',array('id'=>$item['user'][doname]))?>"><?php echo $item['user'][username];?></a>) </span>
</li>
<?php }?>
</ul>
</div>
