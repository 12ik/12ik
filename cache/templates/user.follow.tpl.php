...
<?php foreach((array)$arrFollowUser as $key=>$item) {?>
    <li class="clearfix" id="u<?php echo $item['userid'];?>">
        ...
        <div class="info">
          <h3> <a title="<?php echo $item['username'];?>" href="<?php echo SITE_URL;?><?php echo ikurl('hi','',array('id'=>$item['doname']))?>"><?php echo $item['username'];?></a></h3>
          <!-- 签名档 -->
...
$('.user-list .ban').click(function () {
	...
	if (hasBlackList) {
		$(this).parents('li').fadeOut(function () {
			var posturl = "<?php echo SITE_URL;?><?php echo ikurl('user','do',array('ik'=>'user_nofollow_ajax'))?>";
			$.post_withck(
				posturl,
...
<?php if($strUser['userid'] == $IK_USER['user'][userid]) { ?>
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo SITE_URL;?><?php echo ikurl('user','followed',array('userid'=>$userid))?>">关注我的人(<?php echo $strUser['count_followed'];?>)</a>
        </p> 
<?php } else { ?>
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo SITE_URL;?><?php echo ikurl('user','followed',array('userid'=>$userid))?>">关注<?php echo $strUser['username'];?>的人(<?php echo $strUser['count_followed'];?>)</a>
        </p> 
<?php } ?>               
<!---
        <p class="pl2">
