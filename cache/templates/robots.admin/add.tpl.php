...
.maintable th { background: #f8f9fa; width: 210px; text-align: left; color: #336699; font-weight: normal; }
.maintable td th, .maintable td td { border: none; }
.maintable th p { margin: 0; color: #999; }
...
{	
	var addurl = '<?php echo SITE_URL;?>index.php?app=robots&ac=admin&mg=add&ts=do';
	obj = $(obj);
	if($('#name').val()=='')
	{
...
function debugsubmit(obj, nodename, process) {
	...
	//跳转地址
	$('#theform').attr('action','<?php echo SITE_URL;?>index.php?app=robots&ac=admin&mg=add&ts=debug');	
...
<!--main-->
<div class="midder"> <?php include template("admin/menu");?>
...
<div class="buttons">
    <input type="submit" class="submit" value="提交保存" name="thevaluesubmit" style="padding:8px 10px">&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" value="重置" name="thevaluereset" style="padding:8px 10px; cursor:pointer">
</div>  
