...
<div class="note-item" id="note-<?php echo $noteid;?>">
  ...
  <div class="note-ft" style="" id="note_<?php echo $strNote['contentid'];?>_footer">
        ...
        <a href="<?php echo SITE_URL;?><?php echo ikurl('site','notes',array('ik'=>'edit','notesid'=>$notesid,'noteid'=>$noteid))?>">&gt; 修改</a>
        <a title="真的要删除这篇日记吗？" onClick="return confirm('真的要删除这篇日记吗？')" class="a_confirm_link" href="<?php echo SITE_URL;?><?php echo ikurl('site','notes',array('ik'=>'delnote','notesid'=>$notesid,'noteid'=>$noteid))?>">&gt; 删除</a>
		<?php } ?>
  </div>
</div>
...
<div class="post-comments">
  <div id="comments">
    ...
    <div id="13299013" class="comment-item">
        <div class="pic">
            <a href="<?php echo SITE_URL;?><?php echo ikurl('hi','',array('id'=>$item['user'][doname]))?>"><img alt="<?php echo $item['user'][username];?>" src="<?php echo $item['user'][face];?>"></a>
        </div>
        <div class="content report-comment">
            ...
            <div class="op-lnks">
                ...
                <a title="删除<?php echo $item['user'][username];?>的留言?" class="" href="<?php echo SITE_URL;?><?php echo ikurl('site','notes',array('ik'=>'del_comment','notesid'=>$notesid,'noteid'=>$noteid,'commentid'=>$item['commentid']))?>" rel="nofollow" onClick="return confirm('删除<?php echo $item['user'][username];?>的留言?')">删除</a>
                	<?php } ?>
                <?php } ?>
    ...
    <div id="add_comment" class="comment-form txd">
      <form action="<?php echo SITE_URL;?><?php echo ikurl('site','notes',array('ik'=>'add_comment','notesid'=>$notesid,'noteid'=>$noteid))?>" method="post" name="comment_form">
    ...
    <?php } elseif ($userid>0) { ?>
    <div class="comment-lnk">&gt;&nbsp;
    <a href="<?php echo SITE_URL;?><?php echo ikurl('site','notes',array('notesid'=>$notesid,'noteid'=>$noteid,'goon'=>'1'))?>#add_comment" rel="nofollow">继续发言</a>
    </div>    
    <?php } ?>
  </div>
</div>
