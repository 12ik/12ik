...
<?php foreach((array)$arrFollowedUser as $key=>$item) {?>
    <li class="clearfix" id="u<?php echo $item['userid'];?>">
        ...
        <div class="info">
          <h3>
              <a title="<?php echo $item['username'];?>" href="<?php echo U('hi','',array('id'=>$item['doname']))?>"><?php echo $item['username'];?></a>
...
<?php if($strUser['userid'] == $IK_USER['user'][userid]) { ?>
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('user','follow',array('userid'=>$userid))?>">我关注的人(<?php echo $strUser['count_follow'];?>)</a>
        </p> 
<?php } else { ?>
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('user','follow',array('userid'=>$userid))?>"><?php echo $strUser['username'];?>关注的人(<?php echo $strUser['count_follow'];?>)</a>
        </p> 
<?php } ?>               
<!---
        <p class="pl2">
