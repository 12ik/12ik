...
<div class="cleft">
    <div class="art-body">
        ...
        <div style="text-align:center;color:#999999;padding-bottom: 10px;border-bottom: 1px solid #DDDDDD; margin-bottom:10px"><a href="<?php echo SITE_URL;?><?php echo tsurl('hi','',array('id'=>$strArticle['user'][doname]))?>"><?php echo $strArticle['user'][username];?></a> 发表于 <?php echo date('Y-m-d H:i:s',$strArticle['addtime'])?> <a href="#comments"><?php echo $strArticle['count_comment'];?>条回复</a> 浏览<?php echo $strArticle['count_view'];?>次 <a href="#formMini">我要回复</a> 
        </div>
    ...
    <?php if(intval($IK_USER['user'][userid])==$strArticle['userid']) { ?>
    &gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=note&ac=do&ts=edit&noteid=<?php echo $strArticle['noteid'];?>">修改</a> 
    &gt;&nbsp; <a href="<?php echo SITE_URL;?>index.php?app=note&ac=do&ts=del&noteid=<?php echo $strArticle['noteid'];?>">删除</a>
    <?php } ?>
    </div>
...
<form method="POST" action="<?php echo SITE_URL;?>index.php?app=note&ac=comment&ts=add" onSubmit="return checkComment('#formMini');" id="formMini">
	...
	<input type="hidden" name="noteid" value="<?php echo $strArticle['noteid'];?>" />
...
<div class="cright">
<?php if($strArticle['user'][userid] == $IK_USER['user'][userid] ) { ?>
<p class="pl2"><a href="<?php echo SITE_URL;?><?php echo tsurl('note','list',array('userid'=>$strArticle['user'][userid]))?>">&gt; 返回到我的日志</a></p>
<?php } else { ?>
<p class="pl2"><a href="<?php echo SITE_URL;?><?php echo tsurl('note','list',array('userid'=>$strArticle['user'][userid]))?>">&gt; 返回到<?php echo $strArticle['user'][username];?>的日志</a></p>
<?php } ?>
