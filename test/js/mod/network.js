define(
		"mod/network",
		[ "mod/lang", "mod/browsers" ],
		function(_, browsers, require, exports) {
			var oz = this, ns = _.ns, uuid4jsonp = 1;
			...
			var ajax = function(s) {
				...
				var status, data, requestDone = false, xhr = window.ActiveXObject ? new ActiveXObject(
						"Microsoft.XMLHTTP")
						: new XMLHttpRequest();
				xhr.open(options.type, options.url, true, options.username,
						options.password);
				try {
					if (options.data) {
					...
					xhr
							.setRequestHeader(
									"Accept",
									s.dataType && options.accepts[s.dataType] ? options.accepts[s.dataType]
											+ ", */*"
				...
				function httpSuccess(r) {
					try {
						return !r.status && location.protocol == "file:"
								...
								|| browsers.safari && r.status == undefined
					} catch (e) {
					}
					return false
				}
			...
			exports.getStyle = function(url) {
				var doc = this.document || document, s = doc
						.createElement("link");
			...
			exports.getJSON = function(url, data, fn, op) {
				...
				if (fn) {
					if ((!op || !op.isScript)
							&& (!domain || domain[1] === window.location.host)) {
						ajax({
							url : url,
							data : data,
							success : fn,
				...
				url = [ url, /\?/.test(url) ? "&" : "?", httpParam(data) ]
						.join("");
				if (fn) {
					ns(op.callback, fn)
				}
