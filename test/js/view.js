define("gondor/view", ["mod/lang", "lib/jquery", "host", "mod/browsers", "mod/template", "mod/uiproxy", "mod/mainloop",
 "mod/animate", "mod/easing", "mod/dialog", "mod/editable", "mod/stick", "mod/mention", "gondor/observer", "gondor/toolkit", 
 "gondor/view/uievent", "gondor/view/maplayers", "gondor/view/fullmap", "gondor/view/dashboard", "gondor/view/sidebar", 
 "gondor/view/searchbar", "gondor/view/sonorus", "gondor/view/town", "gondor/view/shop", "gondor/view/house", 
 "gondor/view/widget", "gondor/view/sound", "gondor/view/citizen", "gondor/view/growl", "gondor/view/newbie", 
 "gondor/view/card", "gondor/view/dou", "gondor/view/guide", "gondor/view/dialog", "gondor/view/suggester",
  "gondor:domain"],
function(M, an, K, az, y, ao, a, Q, P, I, q, C, ad, E, O, av, G, al, m, r, i, af, f, t, N, v, b, ak, l, Y, R, p, X, z, J, ap) {
    var g = S("area"),
    ae = S("node"),
    ay = S("place"),
    ab = {
        hd: function(aE, aD) {
            an(aE).find("h6").css("background", aD).end().find(".hd-roof-2").css("background", aD)
        },
        hdBorder: function(aE, aD) {
            an(aE).css("borderColor", aD).find("h6").css("borderColor", aD).end().find(".hd-roof-1").css("background", aD)
        },
    ...
    aa = {
        hd: function(aE, aD) {
            an(aE).find("h6").css("background", aD).end().find(".hd-roof-1,.hd-roof-2").css("background", aD)
        },
        hdBorder: function(aE, aD) {
            an(aE).css("borderColor", aD).find("h6").css("borderColor", aD)
        },
        hdText: function(aE, aD) {
            an(aE).find("h6").css("color", aD)
        }
    ...
    var aA = {
        ...
        ".guide-btn.start": function() {
            var aD = /#nav_id=(\w*)&nav_kind=(\w*)&cid=(\w*)/.exec(this.href) || [];
            E.cancel("fullmap:close", T).fire("viewport:teleport", [aD[1], {
                content_id: aD[3],
                is_place: aD[2] === "place"
            }])
        },
        ".guide-btn.cancel": function() {
            X.clearPath();
            al.clearMark();
            var aD = /#id=(\w*)&kind=(\w*)&place_domain=([\w\-\.]*)&place_kind=(\w*)/.exec(this.href);
            if (aD) {
        ...
        ".theme-btn": function() {
            an(this).addClass("theme-enabled").parent().parent().addClass("theme-enabled").parent().find(".shop-overlap").hide()
        },
        ".theme-btn.theme-enabled": function() {
            var aE = {};
            var aD = /#pid=(\d+)/.exec(this.href)[1];
            var aF = (/place\-\d+\-(\d+)/.exec(this.parentNode.parentNode.parentNode.className) || [])[1];
            an(this).removeClass("theme-enabled").parent().parent().removeClass("theme-enabled").parent().find(".shop-overlap").show().end().find(".theme-panel").find("input").each(function() {
        ...
        ".theme-prop": function() {
            var aF = this,
            aD = this.parentNode.parentNode,
            aE = an(aD).attr("data-shopkind");
            ai.showColorPicker(aF.value, aD,
            function(aG, aJ, aI) {
                var aH = aF.value = "#" + aJ;
                aF.style.backgroundColor = aH; (aE < 2 ? aa: ab)[aF.name](aD.parentNode, aH);
                ai.hideColorPicker()
            })
        },
        ...
        ".door-btn": function() {
            ...
            var aD = /\/home\/([\w\-\.]+)\/#nid=(\d+)/.exec(an(this).attr("href"));
            N.zoom({
                parent_id: f.vision,
                place_domain: aD[1],
                id: aD[2]
        ...
        ".door-btn.sale": function() {
            ...
            var aD = /node\/(\d+)\/resident_op\?op=(\w+)&hid=(\d+)&page=(\d+)/.exec(an(this).attr("href"));
            if (aD) {
        ...
        ".mapnav-close": function() {
            ...
            var aD = /#id=(\w*)&kind=(\w*)&place_domain=([\w\-\.]*)&place_kind=(\w*)/.exec(an(this).attr("href"));
            E.fire("fullmap:return", [{
                id: aD[1],
                kind: aD[2],
                place_domain: aD[3],
        ...
        ".insert-tag.insert-sharp": function() {
            var aD = an(this).closest("form, .note-form").find("textarea.mention");
        ...
        ".edit-card": function() {
            R.hideCard();
            var aE = (/user\/(\d+)/.exec(this.href) || [])[1],
            aD = "/api/user/" + aE + "/update";
            z.set({
                isHideTitle: true,
                iframeURL: aD,
                width: 490,
        ...
        ".editnow-yes": function() {
            var aG = this.parentNode,
            aH = an(".editnow", aG),
            aF = aH.attr("href").replace(/.*#/, ""),
            aD = encodeURIComponent(an(".editnow-input", aG).val()),
            aE = {
                callback: function(aI) {
                    if (!aI.r) {
                        q.toNormal(aH[0], aI.text || aD)
                    } else {
                        ai.alert(aI.msg || "你的修改无法保存，可能是网络问题，或是内容不符合要求")
                    }
                }
            }; (/\?(.*)/.exec(aF) || ["", ""])[1].split("&").forEach(function(aI) {
                if (aI) {
                    aI = aI.split("=");
                    aE[aI[0]] = aI[1] || aD || "%20"
                }
        ...
        ".bookmark-btn": function() {
            E.fire("place:bookmark", [(/pid=(\d+)/.exec(this.href) || [])[1], "add"])
        },
        ...
        ".house-extend-btn": function() {
            var aD = /node\/(\d+)\/resident_op\?op=(\w+)/.exec(this.href);
            if (aD) {
                E.fire("house:op", aD.slice(1))
            }
        },
        ...
        ".loot-dou": function(aG) {
            ...
            E.fire("citizen:pick", /num=(\d+)&para=(.+?)&url=(.*)/.exec(aE.attr("data-opt")).slice(1))
        },
        ...
        ".activity-follow-btn": function() {
            ...
            var aD = this;
            E.fire("activity:follow", [aE[1], aE[2], aE[3],
            function() {
                an(aD).addClass("disabled").html("即将收到提醒").parent().find("em").html(function(aF, aG) {
                    if (/\d/.test(aG)) {
                        return aG.replace(/\d+/,
                        function(aH) {
                            return parseInt(aH, 10) + 1
                        })
                    } else {
                        return "1人感兴趣"
                    }
        ...
        ".activity-vip-list.with_dialog": function() {
            var aD = an(this).parent().find(".viplist").html();
            ai.activityPosterDlg.confirm('<ul class="viplist">' + aD + "</ul>")
        },
        ".mail-view": function() {
    ...
    function j() {
        ...
        var aD = /\/shop\/([\w\-\.]+)\/#nid=(\d+)/.exec(an(this).attr("href"));
        if (aD) {
            t.zoom({
                parent_id: f.vision,
                place_domain: aD[1],
    ...
    function x() {
        var aD = /place\/(\d+)\/widget\/(\d+)\/activity\/(\d+)/.exec(this.href);
    ...
    function at() {}
    function aC() {
        ai.alert("你目前的身份是游客，无法使用这项功能。请先登录。",
        function() {
            setTimeout(function() {
                ai.showLoginbox()
            },
    ...
    var ai = {
        init: function(aD) {
            ...
            this.bookmarkNum = this.userbar.find("strong.bookmarks-view");
            this.mask = this.viewport.find(".globalmask");
            if (!this.mask[0]) {
                this.mask = an('<div class="globalmask"></div>').appendTo(this.viewport)
            }
            var aG = this.viewport.find(".maplayer");
            O.getAssets(aD.assets,
            function() {
            ...
            var aF = this.mainlayer = G(aG[0]),
            aE = this.placelayer = G(aG[1], {
                disable_dashboard: true,
                disable_sidebar: true
            });
            this.mainlayer.event.bind("resize",
            function(aH) {
                m.updateSize();
                r.updateSize();
                al.updateSize()
        ...
        initDashboard: function(aD) {
            if (aD.id) {
                this.userbar.append(y.convertTpl("tplUserbar", aD, "user"));
        ...
        updateDashboard: function(aD) {
            ...
            if (aD.isShopAdmin) {
                this.userbar.find(".shop-opt .menu span").removeClass("disabled")
            }
            if (aD.role === "anonuser") {
        ...
        showBookmark: function(aD) {
            ai.dialog.set({
                isHideTitle: true,
                iframeURL: "/api/user/" + aD + "/collections?action=get&alt=html&start=0&limit=20",
                width: 900,
                buttons: []
            }).open()
        },
        ...
        updateMapNav: function(aF, aD) {
            ...
            "nav");
            document.title = aF.length === 1 ? au: (L(aE[aE.length - 1]) + " - " + au)
        },
        show: function() {
            var aD = this,
            aE = +new Date() - ar;
            ...
            aD.loadingBox.hide().find("div").addClass("app-loadingmask").end().find(".msg").html(s).end()[0].className = this._loadingBoxClassName;
            setTimeout(function() {
                r.enable()
            },
        ...
        showProcess: function(aD) {
            ...
            aD);
            this.loadingBox.find(".msg").html(aD.msg).end().show()[0].className = this._loadingBoxClassName + " " + aD.className;
            ar = ag = +new Date();
        ...
        hideProcess: function(aF) {
            ...
            if (aG < aF.duration) {
                ...
                if (aF.complete_msg || aF.complete_className) {
                    this.loadingBox.addClass(aF.complete_className).find(".msg").html(aF.complete_msg);
                    setTimeout(function() {
                        aE()
                    },
                    aF.delay)
        ...
        showActivityAlert: function(aD) {
            aD.domain = ap;
            ai.alert(y.convertTpl("tplActivityPoster", aD),
            function() {
                ai.host.move("N");
                E.wait("viewport:ready",
                function() {
                    setTimeout(function() {
                        ai.host.move("N")
                    },
        ...
        showActivityPoster: function(aG) {
            ...
            an("body").click(function() {
                if (!aF) {
                    aE.activityPosterDlg.close().node.unbind("click", aD);
        ...
        showDou: function(aF) {
            ...
            var aI = aG.activity[aG.vision],
            aD = an(aG === f ? ".shop": ".edge-S .wall", aI.dom),
            aH = aD.eq(parseInt(Math.random() * aD.length, 10));
            p.show(aH, aF);
            aE.itemsPool.push(p)
        },
        clearItems: function() {
        ...
        showLuckNum: function(aD, aF) {
            var aE = an('<span class="loot-tips">+<span class="refer-price">1</span></span>').css({
                left: aD + "px",
                top: aF + "px"
            }).appendTo("body");
            Q.addStage("luckNum", {
        ...
        alertSNSError: function(aD) {
            ...
            ai.alert("<p>由于授权过期，刚才发表的内容无法分享到" + aE.join("、") + "，请重新授权：</p><p>" + (aD.ap_douban && '<a href="/accounts/oauth_login_request?source=ap_douban" target="_blank" style="margin:0 10px">豆瓣账号授权</a>' || "") + (aD.ap_weibo && '<a href="/accounts/oauth_login_request?source=ap_weibo" target="_blank" style="margin:0 10px">微博账号授权</a>' || "") + "</p>")
        },
        alertAnonuserPost: function() {
            ai.alert("你目前的身份是游客，无法在城内发表任何言论和互动。请先登录。",
            function() {
                setTimeout(function() {
                    ai.showLoginbox()
                },
        ...
        alertArchivedActivity: function(aD) {
            ai.alert(y.convertTpl("tplActivityPoster", aD.activity_info),
            function() {
        ...
        showMyCard: function(aD) {
            E.fire("card:info", [aD, "citizen", this.userbar.find(".myinfo").offset()])
        },
        town: f,
        sound: b,
        shop: t,
