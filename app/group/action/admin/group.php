...
switch($ik){
	...
	case "list":
		...
		$url = SITE_URL.'index.php?app=group&a=admin&mg=group&ik=list&page=';
		$lstart = $page*10-10;
	...
	case "add_do":
		...
		//回到小组管理首页
		header("Location: ".SITE_URL."index.php?app=group&a=admin&mg=group&ik=list");
		break;
	...
	case "isaudit":
		...
		$msg_content = '恭喜你，你申请的小组《'.$strGroup['groupname'].'》审核通过！快去看看吧<br />'.SITE_URL.U('group','show',array('id'=>$groupid));
	...
	case "isrecommend":
		...
		if($strGroup['isrecommend'] == 0){
			...
			$msg_content = '恭喜你，你的小组《'.$strGroup['groupname'].'》被推荐啦！快去看看吧<br />'.SITE_URL.U('group','show',array('id'=>$groupid));
			aac('message')->sendmsg($msg_userid,$msg_touserid,$msg_title,$msg_content);
