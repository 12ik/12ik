...
switch($ik){
	...
	}else if(valid_email($stremail[0]) == false)
	{
		...
		foreach($stremail as $key=>$item)
		{
			...
			$inviteurl = SITE_URL.U('user','invited',array('confirmation'=>$saltmail));
	...
	<a href="'.$inviteurl.'">'.$inviteurl.'</a><br/>如果您愿意先去随便逛逛，点<a href="http://www.12ik.com">http://www.12ik.com</a> 。你的朋友的爱客主页在 '.SITE_URL.U('hi','',array('id'=>$strUser['doname'])).' 。想注册请直接用以上链接，这样你和小麦狼会自动加入彼此的友邻行列。<br/>
