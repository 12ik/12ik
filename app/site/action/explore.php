...
switch ($ik) {
	case "site" :
		...
		$url = SITE_URL."index.php?app=site&a=explore&ik=site&page=";
		$lstart = $page*30-30;
	...
	case "tag" :
		...
		$url = SITE_URL.U('site','site_tag',array('siteid'=>$siteid,'page'=>''));
		...
		foreach($arrSites as $key=>$item){
			$arrSite[] = $item;
			$arrSite[$key]['url'] = SITE_URL.U('site','mine',array('siteid'=>$item[siteid]) );
			$arrSite[$key]['sitedesc'] = getsubstrutf8(t($item['sitedesc']),0,30);
